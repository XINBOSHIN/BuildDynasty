...
import BuildDynasty.api.pathing.movement.MovementStatus;
...
import BuildDynasty.pathing.calc.AbstractNodeCostSearch;
...
import BuildDynasty.pathing.movement.movements.*;
...
import static BuildDynasty.api.pathing.movement.MovementStatus.*;
...
public class PathExecutor implements IPathExecutor, Helper {
     ...
     *
     * @see <a href="https://github.com/cabaletta/BuildDynasty/issues/102">Issue #102</a>
    ...
    public boolean onTick() {
        ...
        if (pathPosition < path.movements().size() - 1) {
            ...
            if (!behavior.BuildDynasty.bsi.worldContainsLoadedChunk(next.getDest().x, next.getDest().z)) {
        ...
        if (costEstimateIndex == null || costEstimateIndex != pathPosition) {
            ...
            for (int i = 1; i < BuildDynasty.settings().costVerificationLookahead.value && pathPosition + i < path.length() - 1; i++) {
        ...
        if (!movement.calculatedWhileLoaded() && currentCost - currentMovementOriginalCostEstimate > BuildDynasty.settings().maxCostIncrease.value && canCancel) {
        ...
        if (movementStatus == SUCCESS) {
            ...
            ticksOnCurrent++;
            if (ticksOnCurrent > currentMovementOriginalCostEstimate + BuildDynasty.settings().movementTimeoutTicks.value) {
    ...
    private boolean shouldSprintNextTick() {
        boolean requested = behavior.BuildDynasty.getInputOverrideHandler().isInputForcedDown(Input.SPRINT);

        // we'll take it from here, no need for minecraft to see we're holding down control and sprint for us
        behavior.BuildDynasty.getInputOverrideHandler().setInputForceState(Input.SPRINT, false);

        // first and foremost, if allowSprint is off, or if we don't have enough hunger, don't try and sprint
        if (!new CalculationContext(behavior.BuildDynasty, false).canSprint) {
            return false;
        }
        ...
        if (current instanceof MovementTraverse && pathPosition < path.length() - 3) {
            ...
            if (next instanceof MovementAscend && sprintableAscend(ctx, (MovementTraverse) current, (MovementAscend) next, path.movements().get(pathPosition + 2))) {
                if (skipNow(ctx, current)) {
                    ...
                    onChangeInPathPosition();
                    onTick();
                    behavior.BuildDynasty.getInputOverrideHandler().setInputForceState(Input.JUMP, true);
                    return true;
                } else {
            ...
            if (pathPosition < path.length() - 2) {
                ...
                if (MovementHelper.canUseFrostWalker(ctx, next.getDest().down())) {
                    ...
                    if (next instanceof MovementTraverse || next instanceof MovementParkour) {
                        boolean couldPlaceInstead = BuildDynasty.settings().allowPlace.value && behavior.BuildDynasty.getInventoryBehavior().hasGenericThrowaway() && next instanceof MovementParkour; // traverse doesn't react fast enough
        ...
        if (current instanceof MovementAscend && pathPosition != 0) {
            ...
            if (prev instanceof MovementDescend && prev.getDirection().up().equals(current.getDirection().down())) {
                ...
                if (ctx.player().posY >= center.getY() - 0.07) {
                    behavior.BuildDynasty.getInputOverrideHandler().setInputForceState(Input.JUMP, false);
                    return true;
                }
            }
        ...
        if (current instanceof MovementFall) {
            ...
            if (data != null) {
                ...
                clearKeys();
                behavior.BuildDynasty.getLookBehavior().updateTarget(RotationUtils.calcRotationFromVec3d(ctx.playerHead(), data.getFirst(), ctx.playerRotations()), false);
                behavior.BuildDynasty.getInputOverrideHandler().setInputForceState(Input.MOVE_FORWARD, true);
                return true;
            }
        }
        return false;
    ...
    private static boolean sprintableAscend(IPlayerContext ctx, MovementTraverse current, MovementAscend next, IMovement nextnext) {
        if (!BuildDynasty.settings().sprintAscends.value) {
            return false;
        }
    ...
    private static boolean canSprintFromDescendInto(IPlayerContext ctx, IMovement current, IMovement next) {
        ...
        return next instanceof MovementDiagonal && BuildDynasty.settings().allowOvershootDiagonalDescend.value;
    }
    ...
    private void clearKeys() {
        ...
        behavior.BuildDynasty.getInputOverrideHandler().clearAllKeys();
    }

    private void cancel() {
        clearKeys();
        behavior.BuildDynasty.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
    ...
    private PathExecutor cutIfTooLong() {
        if (pathPosition > BuildDynasty.settings().maxPathHistoryLength.value) {
            int cutoffAmt = BuildDynasty.settings().pathHistoryCutoffAmount.value;
