...
import BuildDynasty.pathing.movement.MovementState.MovementTarget;
import BuildDynasty.pathing.precompute.Ternary;
...
import static BuildDynasty.pathing.movement.Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP;
import static BuildDynasty.pathing.precompute.Ternary.*;
...
public interface MovementHelper extends ActionCosts, Helper {
    ...
    static boolean avoidBreaking(BlockStateInterface bsi, int x, int y, int z, IBlockState state) {
        ...
        return BuildDynasty.settings().blocksToDisallowBreaking.value.contains(b)
    ...
    static boolean avoidAdjacentBreaking(BlockStateInterface bsi, int x, int y, int z, boolean directlyAbove) {
        ...
        if (!directlyAbove // it is fine to mine a block that has a falling block directly above, this (the cost of breaking the stacked fallings) is included in cost calculations
                ...
                && BuildDynasty.settings().avoidUpdatingFallingBlocks.value // and if the setting is enabled
                && BlockFalling.canFallThrough(bsi.get0(x, y - 1, z))) { // and if it would fall (i.e. it's unsupported)
            return true; // dont break a block that is adjacent to unsupported gravel because it can cause really weird stuff
        }
        if (block instanceof BlockLiquid) {
            if (directlyAbove || BuildDynasty.settings().strictLiquidCheck.value) {
                return true;
            }
    ...
    static Ternary canWalkThroughBlockState(IBlockState state) {
        ...
        if (BuildDynasty.settings().blocksToAvoid.value.contains(block)) {
            return NO;
        }
    ...
    static boolean canWalkThroughPosition(BlockStateInterface bsi, int x, int y, int z, IBlockState state) {
        ...
        if (block instanceof BlockLiquid) {
            ...
            if (BuildDynasty.settings().assumeWalkOnWater.value) {
                return false;
            }
    ...
    static Ternary canWalkOnBlockState(IBlockState state) {
        ...
        if (block == Blocks.LADDER || (block == Blocks.VINE && BuildDynasty.settings().allowVines.value)) { // TODO reconsider this
            return YES;
        }
        ...
        if (MovementHelper.isLava(block) && BuildDynasty.settings().assumeWalkOnLava.value) {
            return MAYBE;
        }
        if (block instanceof BlockSlab) {
            if (!BuildDynasty.settings().allowWalkOnBottomSlab.value) {
    ...
    static boolean canWalkOnPosition(BlockStateInterface bsi, int x, int y, int z, IBlockState state) {
        ...
        if (isWater(block)) {
            ...
            if (MovementHelper.isFlowing(x, y, z, state, bsi) || block == Blocks.FLOWING_WATER) {
                ...
                return isWater(up) && !BuildDynasty.settings().assumeWalkOnWater.value;
            }
        ...
        if (MovementHelper.isLava(block) && !MovementHelper.isFlowing(x, y, z, state, bsi) && BuildDynasty.settings().assumeWalkOnLava.value) { // if we get here it means that assumeWalkOnLava must be true, so put it last
            return true;
        }
    ...
    static void switchToBestToolFor(IPlayerContext ctx, IBlockState b) {
        switchToBestToolFor(ctx, b, new ToolSet(ctx.player()), BuildDynastyAPI.getSettings().preferSilkTouch.value);
    }
    ...
    static void switchToBestToolFor(IPlayerContext ctx, IBlockState b, ToolSet ts, boolean preferSilkTouch) {
        if (BuildDynasty.settings().autoTool.value && !BuildDynasty.settings().assumeExternalAutoTool.value) {
    ...
    static PlaceResult attemptToPlaceABlock(MovementState state, IBuildDynasty BuildDynasty, BlockPos placeAt, boolean preferDown, boolean wouldSneak) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (MovementHelper.canPlaceAgainst(ctx, against1)) {
                if (!((BuildDynasty) BuildDynasty).getInventoryBehavior().selectThrowawayForLocation(false, placeAt.getX(), placeAt.getY(), placeAt.getZ())) { // get ready to place a throwaway block
                ...
                Rotation actual = BuildDynasty.getLookBehavior().getAimProcessor().peekRotation(place);
        ...
        if (ctx.getSelectedBlock().isPresent()) {
            ...
            if (selectedBlock.equals(placeAt) || (MovementHelper.canPlaceAgainst(ctx, selectedBlock) && selectedBlock.offset(side).equals(placeAt))) {
                ...
                ((BuildDynasty) BuildDynasty).getInventoryBehavior().selectThrowawayForLocation(true, placeAt.getX(), placeAt.getY(), placeAt.getZ());
