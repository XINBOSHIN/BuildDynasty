...
public abstract class Movement implements IMovement, MovementHelper {
    ...
    protected final IBuildDynasty BuildDynasty;
    ...
    protected Movement(IBuildDynasty BuildDynasty, BetterBlockPos src, BetterBlockPos dest, BetterBlockPos[] toBreak, BetterBlockPos toPlace) {
    ...
    protected Movement(IBuildDynasty BuildDynasty, BetterBlockPos src, BetterBlockPos dest, BetterBlockPos[] toBreak) {
        this(BuildDynasty, src, dest, toBreak, null);
    }
    ...
    protected boolean playerInValidPosition() {
        return getValidPositions().contains(ctx.playerFeet()) || getValidPositions().contains(((PathingBehavior) BuildDynasty.getPathingBehavior()).pathStart());
    }
    ...
    public MovementStatus update() {
        ...
        currentState.getTarget().getRotation().ifPresent(rotation ->
                BuildDynasty.getLookBehavior().updateTarget(
                        rotation,
                        currentState.getTarget().hasToForceRotations()));
        BuildDynasty.getInputOverrideHandler().clearAllKeys();
        currentState.getInputStates().forEach((input, forced) -> {
            BuildDynasty.getInputOverrideHandler().setInputForceState(input, forced);
        });
    ...
    protected boolean prepared(MovementState state) {
        ...
        for (BetterBlockPos blockPos : positionsToBreak) {
            if (!ctx.world().getEntitiesWithinAABB(EntityFallingBlock.class, new AxisAlignedBB(0, 0, 0, 1, 1.1, 1).offset(blockPos)).isEmpty() && BuildDynasty.settings().pauseMiningForFallingBlocks.value) {
                return false;
            }
