...
public class MovementPillar extends Movement {
    ...
    public MovementPillar(IBuildDynasty BuildDynasty, BetterBlockPos start, BetterBlockPos end) {
        super(BuildDynasty, start, end, new BetterBlockPos[]{start.up(2)}, start);
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (ladder) {
            BlockPos against = vine ? getAgainst(new CalculationContext(BuildDynasty), src) : src.offset(fromDown.getValue(BlockLadder.FACING).getOpposite());
            if (against == null) {
            ...
            if (thePlayer.getPosition0().getX() != from.getX() || thePlayer.getPosition0().getZ() != from.getZ()) {
                BuildDynasty.moveTowardsBlock(from);
            }
             */
            ...
            if (!((BuildDynasty) BuildDynasty).getInventoryBehavior().selectThrowawayForLocation(true, src.x, src.y, src.z)) {
                return state.setStatus(MovementStatus.UNREACHABLE);
            }
            ...
            if (dist > 0.17) {//why 0.17? because it seemed like a good number, that's why
                //[explanation added after BuildDynasty port lol] also because it needs to be less than 0.2 because of the 0.3 sneak limit
