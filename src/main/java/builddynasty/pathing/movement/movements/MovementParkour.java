...
public class MovementParkour extends Movement {
    ...
    private MovementParkour(IBuildDynasty BuildDynasty, BetterBlockPos src, int dist, EnumFacing dir, boolean ascend) {
        super(BuildDynasty, src, src.offset(dir, dist).up(ascend ? 1 : 0), EMPTY, src.offset(dir, dist).down(ascend ? 0 : 1));
        this.direction = dir;
        this.dist = dist;
        this.ascend = ascend;
    }
    ...
    public static MovementParkour cost(CalculationContext context, BetterBlockPos src, EnumFacing direction) {
        ...
        return new MovementParkour(context.getBuildDynasty(), src, dist, direction, res.y > src.y);
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (ctx.playerFeet().equals(dest)) {
            ...
            if (ctx.playerFeet().equals(src.offset(direction)) || ctx.player().posY - src.y > 0.0001) {
                if (BuildDynasty.settings().allowPlace.value // see PR #3775
                        && ((BuildDynasty) BuildDynasty).getInventoryBehavior().hasGenericThrowaway()
                        && !MovementHelper.canWalkOn(ctx, dest.down())
                        && !ctx.player().onGround
                        && MovementHelper.attemptToPlaceABlock(state, BuildDynasty, dest.down(), true, false) == PlaceResult.READY_TO_PLACE
                ) {
