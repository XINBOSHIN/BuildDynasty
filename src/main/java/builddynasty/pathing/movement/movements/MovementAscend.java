...
public class MovementAscend extends Movement {
    ...
    public MovementAscend(IBuildDynasty BuildDynasty, BetterBlockPos src, BetterBlockPos dest) {
        super(BuildDynasty, src, dest, new BetterBlockPos[]{dest, src.up(2), dest.up()}, dest.down());
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (!MovementHelper.canWalkOn(ctx, positionToPlace, jumpingOnto)) {
            ...
            if (MovementHelper.attemptToPlaceABlock(state, BuildDynasty, dest.down(), false, true) == PlaceResult.READY_TO_PLACE) {
        ...
        if (BuildDynasty.settings().assumeStep.value || ctx.playerFeet().equals(src.up())) {
