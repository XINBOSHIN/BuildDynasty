...
package BuildDynasty.pathing.movement.movements;
...
import BuildDynasty.api.utils.VecUtils;
...
public class MovementTraverse extends Movement {
    ...
    public MovementTraverse(IBuildDynasty BuildDynasty, BetterBlockPos from, BetterBlockPos to) {
        super(BuildDynasty, from, to, new BetterBlockPos[]{to.up(), to}, to.down());
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (state.getStatus() != MovementStatus.RUNNING) {
            ...
            if (!BuildDynasty.settings().walkWhileBreaking.value) {
                return state;
            }
        ...
        if (isTheBridgeBlockThere) {
            ...
            if (BuildDynasty.settings().overshootTraverse.value && (feet.equals(dest.add(getDirection())) || feet.equals(dest.add(getDirection()).add(getDirection())))) {
                return state.setStatus(MovementStatus.SUCCESS);
            }
            ...
            if (wasTheBridgeBlockAlwaysThere && (!MovementHelper.isLiquid(ctx, feet) || BuildDynasty.settings().sprintInWater.value) && (!MovementHelper.avoidWalkingInto(intoBelow) || MovementHelper.isWater(intoBelow)) && !MovementHelper.avoidWalkingInto(intoAbove)) {
            ...
            if (feet.getY() != dest.getY() && ladder && (destDown.getBlock() == Blocks.VINE || destDown.getBlock() == Blocks.LADDER)) {
                against = destDown.getBlock() == Blocks.VINE ? MovementPillar.getAgainst(new CalculationContext(BuildDynasty), dest.down()) : dest.offset(destDown.getValue(BlockLadder.FACING).getOpposite());
                if (against == null) {
            ...
            PlaceResult p = MovementHelper.attemptToPlaceABlock(state, BuildDynasty, dest.down(), false, true);
            if ((p == PlaceResult.READY_TO_PLACE || dist1 < 0.6) && !BuildDynasty.settings().assumeSafeWalk.value) {
                state.setInput(Input.SNEAK, true);
            }
            switch (p) {
                case READY_TO_PLACE: {
                    if (ctx.player().isSneaking() || BuildDynasty.settings().assumeSafeWalk.value) {
            ...
            if (feet.equals(dest)) {
                ...
                // Out.log("Trying to look at " + goalLook + ", actually looking at" + BuildDynasty.whatAreYouLookingAt());
