...
import BuildDynasty.utils.IRenderer;
import BuildDynasty.utils.PathRenderer;
import BuildDynasty.utils.accessor.IChunkProviderClient;
import BuildDynasty.utils.accessor.IEntityFireworkRocket;
...
import static BuildDynasty.utils.BuildDynastyMath.fastCeil;
import static BuildDynasty.utils.BuildDynastyMath.fastFloor;
...
public final class ElytraBehavior implements Helper {
    ...
    public ElytraBehavior(BuildDynasty BuildDynasty, ElytraProcess process, BlockPos destination, boolean appendDestination) {
        ...
        this.context = new NetherPathfinderContext(BuildDynasty.settings().elytraNetherSeed.value);
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        if (!this.blockedLines.isEmpty() && BuildDynasty.settings().elytraRenderRaytraces.value) {
        ...
        if (this.simulationLine != null && BuildDynasty.settings().elytraRenderSimulation.value) {
    ...
    public void onReceivePacket(PacketEvent event) {
        if (event.getPacket() instanceof SPacketPlayerPosLook) {
            ctx.minecraft().addScheduledTask(() -> {
                this.remainingSetBackTicks = BuildDynasty.settings().elytraFireworkSetbackUseDelay.value;
            });
        }
    }

    public void pathTo() {
        if (!BuildDynasty.settings().elytraAutoJump.value || ctx.player().isElytraFlying()) {
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > BuildDynasty.settings().elytraTimeBetweenCacheCullSecs.value) {
            this.context.queueCacheCulling(ctx.player().chunkCoordX, ctx.player().chunkCoordZ, BuildDynasty.settings().elytraCacheCullDistance.value, this.boi);
    ...
    /**
     * Called by {@link BuildDynasty.process.ElytraProcess#onTick(boolean, boolean)} when the process is in control and the player is flying
     */
    public void tick() {
        ...
        BuildDynasty.getLookBehavior().updateTarget(solution.rotation, false);
        ...
        for (int relaxation = 0; relaxation < 3; relaxation++) { // try for a strict solution first, then relax more and more (if we're in a corner or near some blocks, it will have to relax its constraints a bit)
            ...
            for (int i = Math.min(playerNear + 20, path.size() - 1); i >= minStep; i--) {
                ...
                for (final Pair<Vec3d, Integer> candidate : candidates) {
                    ...
                    final double minAvoidance = BuildDynasty.settings().elytraMinimumAvoidance.value;
    ...
    private void tickUseFireworks(final Vec3d start, final Vec3d goingTo, final boolean isBoosted, final boolean forceUseFirework) {
        ...
        final boolean useOnDescend = !BuildDynasty.settings().elytraConserveFireworks.value || ctx.player().posY < goingTo.y + 5;
        ...
        final double elytraFireworkSpeed = BuildDynasty.settings().elytraFireworkSpeed.value;
        if (this.remainingFireworkTicks <= 0 && (forceUseFirework || (!isBoosted
            ...
            if (!BuildDynasty.getInventoryBehavior().throwaway(true, ElytraBehavior::isBoostingFireworks) &&
                    !BuildDynasty.getInventoryBehavior().throwaway(true, ElytraBehavior::isFireworks)) {
    ...
    private final class SolverContext {
        ...
        public SolverContext(boolean async) {
            ...
            ITickableAimProcessor aim = ElytraBehavior.this.BuildDynasty.getLookBehavior().getAimProcessor().fork();
            if (async) {
    ...
    private boolean isHitboxClear(final SolverContext context, final Vec3d dest, final Double growAmount) {
        ...
        if (BuildDynasty.settings().elytraRenderHitboxRaytraces.value) {
            boolean clear = true;
            for (int i = 0; i < 8; i++) {
    ...
    public boolean clearView(Vec3d start, Vec3d dest, boolean ignoreLava) {
        ...
        if (BuildDynasty.settings().elytraRenderRaytraces.value) {
    ...
    private static FloatArrayList pitchesToSolveFor(final float goodPitch, final boolean desperate) {
        final float minPitch = desperate ? -90 : Math.max(goodPitch - BuildDynasty.settings().elytraPitchRange.value, -89);
        final float maxPitch = desperate ? 90 : Math.min(goodPitch + BuildDynasty.settings().elytraPitchRange.value, 89);
    ...
    private Pair<Float, Boolean> solvePitch(final SolverContext context, final Vec3d goal, final int relaxation) {
        ...
        final int ticks = desperate ? 3 : context.boost.isBoosted() ? Math.max(5, context.boost.getGuaranteedBoostTicks()) : BuildDynasty.settings().elytraSimulationTicks.value;
    ...
    private void tickInventoryTransactions() {
        if (invTickCountdown <= 0) {
            ...
            if (r != null) {
                r.run();
                invTickCountdown = BuildDynasty.settings().ticksBetweenInventoryMoves.value;
            }
        }
    ...
    private int findGoodElytra() {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (slot.getItem() == Items.ELYTRA && (slot.getItem().getMaxDamage() - slot.getItemDamage()) > BuildDynasty.settings().elytraMinimumDurability.value) {
                return i;
            }
        }
        return -1;
    ...
    private void trySwapElytra() {
        if (!BuildDynasty.settings().elytraAutoSwap.value || !invTransactionQueue.isEmpty()) {
            return;
        }

        ItemStack chest = ctx.player().inventory.armorInventory.get(2);
        if (chest.getItem() != Items.ELYTRA
                || chest.getItem().getMaxDamage() - chest.getItemDamage() > BuildDynasty.settings().elytraMinimumDurability.value) {
            return;
        }
