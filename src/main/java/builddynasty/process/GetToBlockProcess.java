...
public final class GetToBlockProcess extends BuildDynastyProcessHelper implements IGetToBlockProcess {
    ...
    public GetToBlockProcess(BuildDynasty BuildDynasty) {
        super(BuildDynasty);
    }
    ...
    public synchronized PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (knownLocations.isEmpty()) {
            if (BuildDynasty.settings().exploreForBlocks.value && !calcFailed) {
        ...
        if (calcFailed) {
            if (BuildDynasty.settings().blacklistClosestOnFailure.value) {
        ...
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            ...
            BuildDynasty.getExecutor().execute(() -> rescan(current, context));
        }
        if (goal.isInGoal(ctx.playerFeet()) && goal.isInGoal(BuildDynasty.getPathingBehavior().pathStart()) && isSafeToCancel) {
            // we're there
    ...
    public class GetToBlockCalculationContext extends CalculationContext {
        ...
        public GetToBlockCalculationContext(boolean forUseOnAnotherThread) {
            super(GetToBlockProcess.super.BuildDynasty, forUseOnAnotherThread);
        }
    ...
    private Goal createGoal(BlockPos pos) {
        ...
        if (blockOnTopMustBeRemoved(gettingTo.getBlock()) && BuildDynasty.bsi.get0(pos.up()).isBlockNormalCube()) {
    ...
    private boolean rightClick() {
        for (BlockPos pos : knownLocations) {
            ...
            if (reachable.isPresent()) {
                BuildDynasty.getLookBehavior().updateTarget(reachable.get(), true);
                if (knownLocations.contains(ctx.getSelectedBlock().orElse(null))) {
                    BuildDynasty.getInputOverrideHandler().setInputForceState(Input.CLICK_RIGHT, true); // TODO find some way to right click even if we're in an ESC menu
    ...
    private boolean walkIntoInsteadOfAdjacent(Block block) {
        if (!BuildDynasty.settings().enterPortal.value) {
            return false;
        }
    ...
    private boolean rightClickOnArrival(Block block) {
        if (!BuildDynasty.settings().rightClickContainerOnArrival.value) {
            return false;
        }
