...
public final class FarmProcess extends BuildDynastyProcessHelper implements IFarmProcess {
    ...
    public FarmProcess(BuildDynasty BuildDynasty) {
        super(BuildDynasty);
    }
    ...
    public void farm(int range, BlockPos pos) {
        if (pos == null) {
            center = BuildDynasty.getPlayerContext().playerFeet();
        } else {
            center = pos;
        }
        this.range = range;
    ...
    private enum Harvest {
        ...
        SUGARCANE(Blocks.REEDS, null) {
            ...
            public boolean readyToHarvest(World world, BlockPos pos, IBlockState state) {
                if (BuildDynasty.settings().replantCrops.value) {
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (BuildDynasty.settings().replantCrops.value) {
            ...
            scan.add(Blocks.LOG);
            if (BuildDynasty.settings().replantNetherWart.value) {
                scan.add(Blocks.SOUL_SAND);
            }
        }

        if (BuildDynasty.settings().mineGoalUpdateInterval.value != 0 && tickCount++ % BuildDynasty.settings().mineGoalUpdateInterval.value == 0) {
            BuildDynasty.getExecutor().execute(() -> locations = BuildDynastyAPI.getProvider().getWorldScanner().scanChunkRadius(ctx, scan, 256, 10, 10));
        }
        if (locations == null) {
        ...
        for (BlockPos pos : both) {
            ...
            if (rot.isPresent() && isSafeToCancel && BuildDynasty.getInventoryBehavior().throwaway(true, soulsand ? this::isNetherWart : this::isPlantable)) {
        ...
        for (BlockPos pos : openLog) {
            for (EnumFacing dir : EnumFacing.Plane.HORIZONTAL) {
                ...
                if (rot.isPresent() && isSafeToCancel && BuildDynasty.getInventoryBehavior().throwaway(true, this::isCocoa)) {
                    RayTraceResult result = RayTraceUtils.rayTraceTowards(ctx.player(), rot.get(), ctx.playerController().getBlockReachDistance());
        ...
        for (BlockPos pos : bonemealable) {
            Optional<Rotation> rot = RotationUtils.reachable(ctx, pos);
            if (rot.isPresent() && isSafeToCancel && BuildDynasty.getInventoryBehavior().throwaway(true, this::isBoneMeal)) {
                BuildDynasty.getLookBehavior().updateTarget(rot.get(), true);
                if (ctx.isLookingAt(pos)) {
                    BuildDynasty.getInputOverrideHandler().setInputForceState(Input.CLICK_RIGHT, true);
                }
        ...
        if (calcFailed) {
            ...
            if (BuildDynasty.settings().notificationOnFarmFail.value) {
        ...
        if (BuildDynasty.getInventoryBehavior().throwaway(false, this::isPlantable)) {
            for (BlockPos pos : openFarmland) {
                goalz.add(new GoalBlock(pos.up()));
            }
        }
        if (BuildDynasty.getInventoryBehavior().throwaway(false, this::isNetherWart)) {
            for (BlockPos pos : openSoulsand) {
                goalz.add(new GoalBlock(pos.up()));
            }
        }
        if (BuildDynasty.getInventoryBehavior().throwaway(false, this::isCocoa)) {
            for (BlockPos pos : openLog) {
                for (EnumFacing direction : EnumFacing.Plane.HORIZONTAL) {
                    if (ctx.world().getBlockState(pos.offset(direction)).getBlock() instanceof BlockAir) {
        ...
        if (BuildDynasty.getInventoryBehavior().throwaway(false, this::isBoneMeal)) {
            for (BlockPos pos : bonemealable) {
