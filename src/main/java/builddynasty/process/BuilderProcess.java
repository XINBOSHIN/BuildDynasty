...
import BuildDynasty.api.pathing.goals.GoalGetToBlock;
import BuildDynasty.api.process.IBuilderProcess;
import BuildDynasty.api.process.PathingCommand;
import BuildDynasty.api.process.PathingCommandType;
import BuildDynasty.api.schematic.FillSchematic;
import BuildDynasty.api.schematic.ISchematic;
import BuildDynasty.api.schematic.IStaticSchematic;
import BuildDynasty.api.schematic.SubstituteSchematic;
...
import BuildDynasty.utils.schematic.MapArtSchematic;
import BuildDynasty.utils.schematic.SchematicSystem;
import BuildDynasty.utils.schematic.SelectionSchematic;
import BuildDynasty.utils.schematic.format.defaults.LitematicaSchematic;
import BuildDynasty.utils.schematic.litematica.LitematicaHelper;
import BuildDynasty.utils.schematic.schematica.SchematicaHelper;
...
public final class BuilderProcess extends BuildDynastyProcessHelper implements IBuilderProcess {
    ...
    public BuilderProcess(BuildDynasty BuildDynasty) {
        super(BuildDynasty);
    }
    ...
    public void build(String name, ISchematic schematic, Vec3i origin) {
        ...
        if (!BuildDynasty.settings().buildSubstitutes.value.isEmpty()) {
            this.schematic = new SubstituteSchematic(this.schematic, BuildDynasty.settings().buildSubstitutes.value);
        }
        int x = origin.getX();
        int y = origin.getY();
        int z = origin.getZ();
        if (BuildDynasty.settings().schematicOrientationX.value) {
            x += schematic.widthX();
        }
        if (BuildDynasty.settings().schematicOrientationY.value) {
            y += schematic.heightY();
        }
        if (BuildDynasty.settings().schematicOrientationZ.value) {
            z += schematic.lengthZ();
        }
        this.origin = new Vec3i(x, y, z);
        this.paused = false;
        this.layer = BuildDynasty.settings().startAtLayer.value;
        this.stopAtHeight = schematic.heightY();
        if (BuildDynasty.settings().buildOnlySelection.value && buildingSelectionSchematic) {  // currently redundant but safer maybe
            if (BuildDynasty.getSelectionManager().getSelections().length == 0) {
                logDirect("Poor little kitten forgot to set a selection while BuildOnlySelection is true");
                this.stopAtHeight = 0;
            } else if (BuildDynasty.settings().buildInLayers.value) {
                OptionalInt minim = Stream.of(BuildDynasty.getSelectionManager().getSelections()).mapToInt(sel -> sel.min().y).min();
                OptionalInt maxim = Stream.of(BuildDynasty.getSelectionManager().getSelections()).mapToInt(sel -> sel.max().y).max();
                if (minim.isPresent() && maxim.isPresent()) {
                    int startAtHeight = BuildDynasty.settings().layerOrder.value ? y + schematic.heightY() - maxim.getAsInt() : minim.getAsInt() - y;
                    this.stopAtHeight = (BuildDynasty.settings().layerOrder.value ? y + schematic.heightY() - minim.getAsInt() : maxim.getAsInt() - y) + 1;
                    this.layer = Math.max(this.layer, startAtHeight / BuildDynasty.settings().layerHeight.value);  // startAtLayer or startAtHeight, whichever is highest
    ...
    private ISchematic applyMapArtAndSelection(Vec3i origin, IStaticSchematic parsed) {
        ...
        if (BuildDynasty.settings().mapArtMode.value) {
            schematic = new MapArtSchematic(parsed);
        }
        if (BuildDynasty.settings().buildOnlySelection.value) {
            schematic = new SelectionSchematic(schematic, origin, BuildDynasty.getSelectionManager().getSelections());
        }
        return schematic;
    }
    ...
    public void buildOpenSchematic() {
        if (SchematicaHelper.isSchematicaPresent()) {
            ...
            if (schematic.isPresent()) {
                ...
                ISchematic schem = BuildDynasty.settings().mapArtMode.value ? new MapArtSchematic(s) : s;
                if (BuildDynasty.settings().buildOnlySelection.value) {
                    schem = new SelectionSchematic(schem, origin, BuildDynasty.getSelectionManager().getSelections());
                }
                this.build(
    ...
    private Optional<Tuple<BetterBlockPos, Rotation>> toBreakNearPlayer(BuilderCalculationContext bcc) {
        ...
        BetterBlockPos pathStart = BuildDynasty.getPathingBehavior().pathStart();
        for (int dx = -5; dx <= 5; dx++) {
            for (int dy = BuildDynasty.settings().breakFromAbove.value ? -1 : 0; dy <= 5; dy++) {
    ...
    private Optional<Placement> possibleToPlace(IBlockState toPlace, int x, int y, int z, BlockStateInterface bsi) {
        for (EnumFacing against : EnumFacing.values()) {
            ...
            for (Vec3d placementMultiplier : aabbSideMultipliers(against)) {
                ...
                Rotation actualRot = BuildDynasty.getLookBehavior().getAimProcessor().peekRotation(rot);
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel, int recursions) {
        ...
        if (BuildDynasty.getInputOverrideHandler().isInputForcedDown(Input.CLICK_LEFT)) {
            ticks = 5;
        } else {
            ticks--;
        }
        ...
        if (BuildDynasty.settings().buildInLayers.value) {
            ...
            if (BuildDynasty.settings().layerOrder.value) { // top to bottom
                maxYInclusive = realSchematic.heightY() - 1;
                minYInclusive = realSchematic.heightY() - layer * BuildDynasty.settings().layerHeight.value;
            } else {
                maxYInclusive = layer * BuildDynasty.settings().layerHeight.value - 1;
                minYInclusive = 0;
            }
        ...
        if (!recalc(bcc)) {
            if (BuildDynasty.settings().buildInLayers.value && layer * BuildDynasty.settings().layerHeight.value < stopAtHeight) {
                logDirect("Starting layer " + layer);
                layer++;
                return onTick(calcFailed, isSafeToCancel, recursions + 1);
            }
            Vec3i repeat = BuildDynasty.settings().buildRepeat.value;
            int max = BuildDynasty.settings().buildRepeatCount.value;
            numRepeats++;
            if (repeat.equals(new Vec3i(0, 0, 0)) || (max != -1 && numRepeats >= max)) {
                logDirect("Done building");
                if (BuildDynasty.settings().notificationOnBuildFinished.value) {
            ...
            if (!BuildDynasty.settings().buildRepeatSneaky.value) {
                schematic.reset();
            }
            logDirect("Repeating build in vector " + repeat + ", new origin is " + origin);
            return onTick(calcFailed, isSafeToCancel, recursions + 1);
        }
        if (BuildDynasty.settings().distanceTrim.value) {
            trim();
        }
        ...
        if (toBreak.isPresent() && isSafeToCancel && ctx.player().onGround) {
            ...
            BuildDynasty.getLookBehavior().updateTarget(rot, true);
            ...
            if (ctx.isLookingAt(pos) || ctx.playerRotations().isReallyCloseTo(rot)) {
                BuildDynasty.getInputOverrideHandler().setInputForceState(Input.CLICK_LEFT, true);
            }
            return new PathingCommand(null, PathingCommandType.CANCEL_AND_SET_GOAL);
        }
        ...
        if (BuildDynasty.settings().allowInventory.value) {
            ...
            for (int i = 9; i < 36; i++) {
                for (IBlockState desired : noValidHotbarOption) {
                    if (valid(approxPlaceable.get(i), desired, true)) {
                        if (!BuildDynasty.getInventoryBehavior().attemptToPutOnHotbar(i, usefulSlots::contains)) {
        ...
        if (goal == null) {
            ...
            if (goal == null) {
                if (BuildDynasty.settings().skipFailedLayers.value && BuildDynasty.settings().buildInLayers.value && layer * BuildDynasty.settings().layerHeight.value < realSchematic.heightY()) {
    ...
    private void recalcNearby(BuilderCalculationContext bcc) {
        BetterBlockPos center = ctx.playerFeet();
        int radius = BuildDynasty.settings().builderTickScanRadius.value;
                ...
                for (int x = 0; x < schematic.widthX(); x++) {
                    ...
                    if (bcc.bsi.worldContainsLoadedChunk(blockX, blockZ)) { // check if its in render distance, not if its in cache
                        ...
                        if (valid(bcc.bsi.get0(blockX, blockY, blockZ), schematic.desiredState(x, y, z, current, this.approxPlaceable), false)) {
                            ...
                            if (incorrectPositions.size() > BuildDynasty.settings().incorrectSize.value) {
                                return;
                            }
                        }
                        continue;
                    ...
                    if (!observedCompleted.contains(BetterBlockPos.longHash(blockX, blockY, blockZ))
                            && !BuildDynasty.settings().buildSkipBlocks.value.contains(schematic.desiredState(x, y, z, current, this.approxPlaceable).getBlock())) {
    ...
    private Goal breakGoal(BlockPos pos, BuilderCalculationContext bcc) {
        if (BuildDynasty.settings().goalBreakFromAbove.value && bcc.bsi.get0(pos.up()).getBlock() instanceof BlockAir && bcc.bsi.get0(pos.up(2)).getBlock() instanceof BlockAir) { // TODO maybe possible without the up(2) check?
    ...
    public void onLostControl() {
        ...
        schematic = null;
        realSchematic = null;
        layer = BuildDynasty.settings().startAtLayer.value;
        numRepeats = 0;
        paused = false;
    ...
    private boolean sameBlockstate(IBlockState first, IBlockState second) {
        ...
        boolean ignoreDirection = BuildDynasty.settings().buildIgnoreDirection.value;
        List<String> ignoredProps = BuildDynasty.settings().buildIgnoreProperties.value;
    ...
    private boolean valid(IBlockState current, IBlockState desired, boolean itemVerify) {
        ...
        if (current.getBlock() instanceof BlockLiquid && BuildDynasty.settings().okIfWater.value) {
            return true;
        }
        if (current.getBlock() instanceof BlockAir && BuildDynasty.settings().okIfAir.value.contains(desired.getBlock())) {
            return true;
        }
        if (desired.getBlock() instanceof BlockAir && BuildDynasty.settings().buildIgnoreBlocks.value.contains(current.getBlock())) {
            return true;
        }
        if (!(current.getBlock() instanceof BlockAir) && BuildDynasty.settings().buildIgnoreExisting.value && !itemVerify) {
            return true;
        }
        if (BuildDynasty.settings().buildSkipBlocks.value.contains(desired.getBlock()) && !itemVerify) {
            return true;
        }
        if (BuildDynasty.settings().buildValidSubstitutes.value.getOrDefault(desired.getBlock(), Collections.emptyList()).contains(current.getBlock()) && !itemVerify) {
            return true;
        }
    ...
    public class BuilderCalculationContext extends CalculationContext {
        ...
        public BuilderCalculationContext() {
            super(BuilderProcess.this.BuildDynasty, true); // wew lad
        ...
        public double costOfPlacingAt(int x, int y, int z, IBlockState current) {
            ...
            if (sch != null && !BuildDynasty.settings().buildSkipBlocks.value.contains(sch.getBlock())) {
        ...
        public double breakCostMultiplierAt(int x, int y, int z, IBlockState current) {
            ...
            if (sch != null && !BuildDynasty.settings().buildSkipBlocks.value.contains(sch.getBlock())) {
                ...
                if (valid(bsi.get0(x, y, z), sch, false)) {
                    return BuildDynasty.settings().breakCorrectBlockPenaltyMultiplier.value;
                } else {
