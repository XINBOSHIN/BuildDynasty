...
import BuildDynasty.api.pathing.goals.*;
import BuildDynasty.api.process.IMineProcess;
 ...
 * @author leijurv
 */
public final class MineProcess extends BuildDynastyProcessHelper implements IMineProcess {
    ...
    public MineProcess(BuildDynasty BuildDynasty) {
        super(BuildDynasty);
    }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (calcFailed) {
            if (!knownOreLocations.isEmpty() && BuildDynasty.settings().blacklistClosestOnFailure.value) {
                logDirect("Unable to find any path to " + filter + ", blacklisting presumably unreachable closest instance...");
                if (BuildDynasty.settings().notificationOnMineFail.value) {
        ...
        updateLoucaSystem();
        int mineGoalUpdateInterval = BuildDynasty.settings().mineGoalUpdateInterval.value;
        List<BlockPos> curr = new ArrayList<>(knownOreLocations);
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            CalculationContext context = new CalculationContext(BuildDynasty, true);
            BuildDynasty.getExecutor().execute(() -> rescan(curr, context));
        }
        if (BuildDynasty.settings().legitMine.value) {
            if (!addNearby()) {
                cancel();
                return null;
            }
        ...
        if (shaft.isPresent() && ctx.player().onGround) {
            ...
            IBlockState state = BuildDynasty.bsi.get0(pos);
            if (!MovementHelper.avoidBreaking(BuildDynasty.bsi, pos.getX(), pos.getY(), pos.getZ(), state)) {
                Optional<Rotation> rot = RotationUtils.reachable(ctx, pos);
                if (rot.isPresent() && isSafeToCancel) {
                    BuildDynasty.getLookBehavior().updateTarget(rot.get(), true);
    ...
    private void updateLoucaSystem() {
        ...
        ctx.getSelectedBlock().ifPresent(pos -> {
            if (knownOreLocations.contains(pos)) {
                copy.put(pos, System.currentTimeMillis() + BuildDynasty.settings().mineDropLoiterDurationMSThanksLouca.value);
            }
        });
    ...
    private PathingCommand updateGoal() {
        ...
        boolean legit = BuildDynasty.settings().legitMine.value;
        List<BlockPos> locs = knownOreLocations;
        if (!locs.isEmpty()) {
            CalculationContext context = new CalculationContext(BuildDynasty);
        ...
        if (!legit && !BuildDynasty.settings().exploreForBlocks.value) {
            return null;
        }
        // only when we should explore for blocks or are in legit mode we do this
        int y = BuildDynasty.settings().legitMineYLevel.value;
        if (branchPoint == null) {
            /*if (!BuildDynasty.getPathingBehavior().isPathing() && playerFeet().y == y) {
    ...
    private void rescan(List<BlockPos> already, CalculationContext context) {
        ...
        locs.addAll(dropped);
        if (locs.isEmpty() && !BuildDynasty.settings().exploreForBlocks.value) {
    ...
    private boolean internalMiningGoal(BlockPos pos, CalculationContext context, List<BlockPos> locs) {
        ...
        if (BuildDynasty.settings().internalMiningAirException.value && state.getBlock() instanceof BlockAir) {
            return true;
        }
    ...
    private Goal coalesce(BlockPos loc, List<BlockPos> locs, CalculationContext context) {
        boolean assumeVerticalShaftMine = !(BuildDynasty.bsi.get0(loc.up()).getBlock() instanceof BlockFalling);
        if (!BuildDynasty.settings().forceInternalMining.value) {
    ...
    public List<BlockPos> droppedItemsScan() {
        if (!BuildDynasty.settings().mineScanDroppedItems.value) {
    ...
    public static List<BlockPos> searchWorld(CalculationContext ctx, BlockOptionalMetaLookup filter, int max, List<BlockPos> alreadyKnown, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        for (BlockOptionalMeta bom : filter.blocks()) {
            ...
            if (CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.contains(block)) {
                BetterBlockPos pf = ctx.BuildDynasty.getPlayerContext().playerFeet();

                // maxRegionDistanceSq 2 means adjacent directly or adjacent diagonally; nothing further than that
                locs.addAll(ctx.worldData.getCachedWorld().getLocationsOf(
                        BlockUtils.blockToString(block),
                        BuildDynasty.settings().maxCachedWorldScanCount.value,
                        pf.x,
                        pf.z,
                        2
                ));
        ...
        if (!untracked.isEmpty() || (BuildDynasty.settings().extendCacheOnThreshold.value && locs.size() < max)) {
            locs.addAll(BuildDynastyAPI.getProvider().getWorldScanner().scanChunkRadius(
                    ctx.getBuildDynasty().getPlayerContext(),
                    filter,
                    max,
                    10,
                    32
    ...
    private boolean addNearby() {
            ...
            for (int y = playerFeet.getY() - searchDist; y <= playerFeet.getY() + searchDist; y++) {
                for (int z = playerFeet.getZ() - searchDist; z <= playerFeet.getZ() + searchDist; z++) {
                    ...
                    if (filter.has(bsi.get0(x, y, z))) {
                        ...
                        if ((BuildDynasty.settings().legitMineIncludeDiagonals.value && knownOreLocations.stream().anyMatch(ore -> ore.distanceSq(pos) <= 2 /* sq means this is pytha dist <= sqrt(2) */)) || RotationUtils.reachable(ctx, pos, fakedBlockReachDistance).isPresent()) {
        ...
        knownOreLocations = prune(new CalculationContext(BuildDynasty), knownOreLocations, filter, ORE_LOCATIONS_COUNT, blacklist, dropped);
        return true;
    }
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter, int max, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        List<BlockPos> locs = locs2
                ...
                .filter(pos -> {
                    if (BuildDynasty.settings().allowOnlyExposedOres.value) {
                ...
                .filter(pos -> pos.getY() >= BuildDynasty.settings().minYLevelWhileMining.value)

                .filter(pos -> pos.getY() <= BuildDynasty.settings().maxYLevelWhileMining.value)

                .filter(pos -> !blacklist.contains(pos))

                .sorted(Comparator.comparingDouble(ctx.getBuildDynasty().getPlayerContext().player()::getDistanceSq))
    ...
    public static boolean isNextToAir(CalculationContext ctx, BlockPos pos) {
        int radius = BuildDynasty.settings().allowOnlyExposedOresDistance.value;
    ...
    public void mine(int quantity, BlockOptionalMetaLookup filter) {
        ...
        if (filter != null) {
            rescan(new ArrayList<>(), new CalculationContext(BuildDynasty));
        }
    }
    ...
    private BlockOptionalMetaLookup filterFilter() {
        ...
        if (!BuildDynasty.settings().allowBreak.value) {
            BlockOptionalMetaLookup f = new BlockOptionalMetaLookup(this.filter.blocks()
                    .stream()
                    .filter(e -> BuildDynasty.settings().allowBreakAnyway.value.contains(e.getBlock()))
