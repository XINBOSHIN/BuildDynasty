...
import BuildDynasty.api.utils.PathCalculationResult;
import BuildDynasty.api.utils.interfaces.IGoalRenderPos;
import BuildDynasty.pathing.calc.AStarPathFinder;
...
import BuildDynasty.utils.pathing.Favoring;
...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public PathingBehavior(BuildDynasty BuildDynasty) {
        super(BuildDynasty);
    }
    ...
    private void dispatchEvents() {
        ...
        for (PathEvent event : curr) {
            BuildDynasty.getGameEventHandler().onPathEvent(event);
        }
    }
    ...
    public void onTick(TickEvent event) {
        ...
        if (event.getType() == TickEvent.Type.OUT) {
            ...
            BuildDynasty.getPathingControlManager().cancelEverything();
            return;
        }

        expectedSegmentStart = pathStart();
        BuildDynasty.getPathingControlManager().preTick();
        tickPath();
        ticksElapsedSoFar++;
        dispatchEvents();
    }
    ...
    private void tickPath() {
        ...
        if (pauseRequestedLastTick && safeToCancel) {
            ...
            if (unpausedLastTick) {
                ...
                BuildDynasty.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
            }
        ...
        synchronized (pathPlanLock) {
            ...
            if (BuildDynasty.settings().splicePath.value) {
            ...
            synchronized (pathCalcLock) {
                ...
                if (ticksRemainingInSegment(false).get() < BuildDynasty.settings().planningTickLookahead.value) {
    ...
    public boolean secretInternalSetGoalAndPath(PathingCommand command) {
        ...
        if (command instanceof PathingCommandContext) {
            ...
            context = new CalculationContext(BuildDynasty, true);
        }
        if (goal == null) {
            return false;
        }
    ...
    public boolean isSafeToCancel() {
        if (current == null) {
            return !BuildDynasty.getElytraProcess().isActive() || BuildDynasty.getElytraProcess().isSafeToCancel();
        }
        return safeToCancel;
    }
    ...
    public boolean cancelEverything() {
        ...
        BuildDynasty.getPathingControlManager().cancelEverything(); // regardless of if we can stop the current segment, we can still stop the processes
        return doIt;
    }
    ...
    private void findPathInNewThread(final BlockPos start, final boolean talkAboutIt, CalculationContext context) {
        ...
        long failureTimeout;
        if (current == null) {
            primaryTimeout = BuildDynasty.settings().primaryTimeoutMS.value;
            failureTimeout = BuildDynasty.settings().failureTimeoutMS.value;
        } else {
            primaryTimeout = BuildDynasty.settings().planAheadPrimaryTimeoutMS.value;
            failureTimeout = BuildDynasty.settings().planAheadFailureTimeoutMS.value;
        }
    ...
    private static AbstractNodeCostSearch createPathfinder(BlockPos start, Goal goal, IPath previous, CalculationContext context) {
        ...
        if (BuildDynasty.settings().simplifyUnloadedYCoord.value && goal instanceof IGoalRenderPos) {
            BlockPos pos = ((IGoalRenderPos) goal).getGoalPos();
            if (!context.bsi.worldContainsLoadedChunk(pos.getX(), pos.getZ())) {
                transformed = new GoalXZ(pos.getX(), pos.getZ());
            }
        }
        Favoring favoring = new Favoring(context.getBuildDynasty().getPlayerContext(), previous, context);
