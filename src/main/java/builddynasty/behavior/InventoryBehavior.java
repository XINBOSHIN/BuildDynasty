...
public final class InventoryBehavior extends Behavior implements Helper {
    ...
    public InventoryBehavior(BuildDynasty BuildDynasty) {
        super(BuildDynasty);
    }
    ...
    private boolean requestSwapWithHotBar(int inInventory, int inHotbar) {
        ...
        if (ticksSinceLastInventoryMove < BuildDynasty.settings().ticksBetweenInventoryMoves.value) {
            logDebug("Inventory move requested but delaying " + ticksSinceLastInventoryMove + " " + BuildDynasty.settings().ticksBetweenInventoryMoves.value);
            return false;
        }
        if (BuildDynasty.settings().inventoryMoveOnlyIfStationary.value && !BuildDynasty.getInventoryPauserProcess().stationaryForInventoryMove()) {
    ...
    private int firstValidThrowaway() { // TODO offhand idk
        ...
        for (int i = 0; i < invy.size(); i++) {
            if (BuildDynasty.settings().acceptableThrowawayItems.value.contains(invy.get(i).getItem())) {
                return i;
            }
        }
        return -1;
    ...
    private int bestToolAgainst(Block against, Class<? extends ItemTool> cla$$) {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (BuildDynasty.settings().itemSaver.value && (stack.getItemDamage() + BuildDynasty.settings().itemSaverThreshold.value) >= stack.getMaxDamage() && stack.getMaxDamage() > 1) {
                continue;
            }
    ...
    public boolean hasGenericThrowaway() {
        for (Item item : BuildDynasty.settings().acceptableThrowawayItems.value) {
    ...
    public boolean selectThrowawayForLocation(boolean select, int x, int y, int z) {
        IBlockState maybe = BuildDynasty.getBuilderProcess().placeAt(x, y, z, BuildDynasty.bsi.get0(x, y, z));
    ...
    public boolean throwaway(boolean select, Predicate<? super ItemStack> desired) {
        return throwaway(select, desired, BuildDynasty.settings().allowInventory.value);
    }
