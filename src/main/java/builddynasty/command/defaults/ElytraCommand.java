...
public class ElytraCommand extends Command {
    ...
    public ElytraCommand(IBuildDynasty BuildDynasty) {
        super(BuildDynasty, "elytra");
    }

    @Override
    public void execute(String label, IArgConsumer args) throws CommandException {
        final ICustomGoalProcess customGoalProcess = BuildDynasty.getCustomGoalProcess();
        final IElytraProcess elytra = BuildDynasty.getElytraProcess();
        ...
        if (!args.hasAny()) {
            if (BuildDynasty.settings().elytraTermsAccepted.value) {
                if (detectOn2b2t()) {
                    warn2b2t();
                }
            } else {
    ...
    private void warn2b2t() {
        if (BuildDynasty.settings().elytraPredictTerrain.value) {
            long seed = BuildDynasty.settings().elytraNetherSeed.value;
    ...
    private ITextComponent suggest2b2tSeeds() {
        ...
        clippy.appendText("Within a few hundred blocks of spawn/axis/highways/etc, the terrain is too fragmented to be predictable. BuildDynasty Elytra will still work, just with backtracking. ");
        ...
        olderSeed.getStyle().setUnderlined(true).setBold(true).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString(BuildDynasty.settings().prefix.value + "set elytraNetherSeed " + OLD_2B2T_SEED))).setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, FORCE_COMMAND_PREFIX + "set elytraNetherSeed " + OLD_2B2T_SEED));
        ...
        newerSeed.getStyle().setUnderlined(true).setBold(true).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString(BuildDynasty.settings().prefix.value + "set elytraNetherSeed " + NEW_2B2T_SEED))).setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, FORCE_COMMAND_PREFIX + "set elytraNetherSeed " + NEW_2B2T_SEED));
    ...
    private void gatekeep() {
        ...
        gatekeep.appendText("BuildDynasty Elytra is an experimental feature. It is only intended for long distance travel in the Nether using fireworks for vanilla boost. It will not work with any other mods (\"hacks\") for non-vanilla boost. ");
        TextComponentString gatekeep2 = new TextComponentString("If you want BuildDynasty to attempt to take off from the ground for you, you can enable the elytraAutoJump setting (not advisable on laggy servers!). ");
        gatekeep2.getStyle().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString(BuildDynasty.settings().prefix.value + "set elytraAutoJump true")));
        gatekeep.appendSibling(gatekeep2);
        TextComponentString gatekeep3 = new TextComponentString("If you want BuildDynasty to go slower, enable the elytraConserveFireworks setting and/or decrease the elytraFireworkSpeed setting. ");
        gatekeep3.getStyle().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString(BuildDynasty.settings().prefix.value + "set elytraConserveFireworks true\n" + BuildDynasty.settings().prefix.value + "set elytraFireworkSpeed 0.6\n(the 0.6 number is just an example, tweak to your liking)")));
        gatekeep.appendSibling(gatekeep3);
        TextComponentString gatekeep4 = new TextComponentString("BuildDynasty Elytra ");
        ...
        if (detectOn2b2t()) {
            ...
            if (!BuildDynasty.settings().elytraPredictTerrain.value) {
                gatekeep5.appendText(BuildDynasty.settings().prefix.value + "elytraPredictTerrain is currently disabled. ");
            } else {
                if (BuildDynasty.settings().elytraNetherSeed.value == NEW_2B2T_SEED) {
                    gatekeep5.appendText("You are using the newer seed. ");
                } else if (BuildDynasty.settings().elytraNetherSeed.value == OLD_2B2T_SEED) {
                    gatekeep5.appendText("You are using the older seed. ");
                } else {
                    gatekeep5.appendText("Defaulting to the newer seed. ");
                    BuildDynasty.settings().elytraNetherSeed.value = NEW_2B2T_SEED;
                }
            }
            gatekeep.appendSibling(gatekeep5);
        } else {
            if (BuildDynasty.settings().elytraNetherSeed.value == NEW_2B2T_SEED) {
                TextComponentString gatekeep5 = new TextComponentString("BuildDynasty doesn't know the seed of your world. Set it with: " + BuildDynasty.settings().prefix.value + "set elytraNetherSeed seedgoeshere\n");
                gatekeep5.appendText("For the time being, elytraPredictTerrain is defaulting to false since the seed is unknown.");
                gatekeep.appendSibling(gatekeep5);
                BuildDynasty.settings().elytraPredictTerrain.value = false;
            } else {
                if (BuildDynasty.settings().elytraPredictTerrain.value) {
                    TextComponentString gatekeep5 = new TextComponentString("BuildDynasty Elytra is predicting terrain assuming that " + BuildDynasty.settings().elytraNetherSeed.value + " is the correct seed. Change that with " + BuildDynasty.settings().prefix.value + "set elytraNetherSeed seedgoeshere, or disable it with " + BuildDynasty.settings().prefix.value + "set elytraPredictTerrain false");
                    gatekeep.appendSibling(gatekeep5);
                } else {
                    TextComponentString gatekeep5 = new TextComponentString("BuildDynasty Elytra is not predicting terrain. If you don't know the seed, this is the correct thing to do. If you do know the seed, input it with " + BuildDynasty.settings().prefix.value + "set elytraNetherSeed seedgoeshere, and then enable it with " + BuildDynasty.settings().prefix.value + "set elytraPredictTerrain true");
                    gatekeep.appendSibling(gatekeep5);
                }
            }
        }
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The elytra command tells BuildDynasty to, in the nether, automatically fly to the current goal.",
                "",
                "Usage:",
                ...
                "> elytra supported - Tells you if BuildDynasty ships a native library that is compatible with your PC."
        );
    }
