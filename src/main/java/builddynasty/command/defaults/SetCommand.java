...
import static BuildDynasty.api.utils.SettingsUtil.*;

public class SetCommand extends Command {

    public SetCommand(IBuildDynasty BuildDynasty) {
        super(BuildDynasty, "set", "setting", "settings");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (Arrays.asList("s", "save").contains(arg)) {
            SettingsUtil.save(BuildDynasty.settings());
        ...
        if (Arrays.asList("load", "ld").contains(arg)) {
            ...
            // reset to defaults
            SettingsUtil.modifiedSettings(BuildDynasty.settings()).forEach(Settings.Setting::reset);
            // then load from disk
            SettingsUtil.readAndApply(BuildDynasty.settings(), file);
        ...
        if (paginate) {
            ...
            List<? extends Settings.Setting> toPaginate =
                    (viewModified ? SettingsUtil.modifiedSettings(BuildDynasty.settings()) : BuildDynasty.settings().allSettings).stream()
            ...
            Paginator.paginate(
                    ...
                    setting -> {
                        ...
                        String commandSuggestion = BuildDynasty.settings().prefix.value + String.format("set %s ", setting.getName());
        ...
        Settings.Setting<?> setting = BuildDynasty.settings().allSettings.stream()
        ...
        if (!doingSomething && !args.hasAny()) {
            ...
            if (resetting) {
                ...
                try {
                    SettingsUtil.parseAndApply(BuildDynasty.settings(), arg, newValue);
                } catch (Throwable t) {
                    t.printStackTrace();
            ...
            if ((setting.getName().equals("chatControl") && !(Boolean) setting.value && !BuildDynasty.settings().chatControlAnyway.value) ||
                    setting.getName().equals("chatControlAnyway") && !(Boolean) setting.value && !BuildDynasty.settings().chatControl.value) {
        ...
        if (args.hasAny()) {
            ...
            if (args.hasExactlyOne() && !Arrays.asList("s", "save").contains(args.peekString().toLowerCase(Locale.US))) {
                if (arg.equalsIgnoreCase("reset")) {
                    ...
                    return RelativeFile.tabComplete(args, Minecraft.getMinecraft().gameDir.toPath().resolve("BuildDynasty").toFile());
                }
                Settings.Setting setting = BuildDynasty.settings().byLowerName.get(arg.toLowerCase(Locale.US));
                if (setting != null) {
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "Using the set command, you can manage all of BuildDynasty's settings. Almost every aspect is controlled by these settings - go wild!",
                "",
                "Usage:",
                ...
                "> set load [filename] - Load settings from another file in your minecraft/BuildDynasty"
        );
    }
}
