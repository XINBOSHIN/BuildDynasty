...
import BuildDynasty.api.command.datatypes.ForAxis;
import BuildDynasty.api.command.datatypes.ForBlockOptionalMeta;
import BuildDynasty.api.command.datatypes.ForEnumFacing;
...
import BuildDynasty.api.event.events.RenderEvent;
import BuildDynasty.api.event.listener.AbstractGameEventListener;
import BuildDynasty.api.schematic.*;
import BuildDynasty.api.schematic.mask.shape.CylinderMask;
import BuildDynasty.api.schematic.mask.shape.SphereMask;
...
import BuildDynasty.api.utils.BlockOptionalMetaLookup;
import BuildDynasty.utils.BlockStateInterface;
import BuildDynasty.utils.IRenderer;
import BuildDynasty.utils.schematic.StaticSchematic;
...
public class SelCommand extends Command {
    ...
    private ISelectionManager manager = BuildDynasty.getSelectionManager();
    private BetterBlockPos pos1 = null;
    private ISchematic clipboard = null;
    private Vec3i clipboardOffset = null;

    public SelCommand(IBuildDynasty BuildDynasty) {
        super(BuildDynasty, "sel", "selection", "s");
        BuildDynasty.getGameEventHandler().registerEventListener(new AbstractGameEventListener() {
            @Override
            public void onRenderPass(RenderEvent event) {
                if (!BuildDynasty.settings().renderSelectionCorners.value || pos1 == null) {
                    return;
                }
                Color color = BuildDynasty.settings().colorSelectionPos1.value;
                float opacity = BuildDynasty.settings().selectionOpacity.value;
                float lineWidth = BuildDynasty.settings().selectionLineWidth.value;
                boolean ignoreDepth = BuildDynasty.settings().renderSelectionIgnoreDepth.value;
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.POS1 || action == Action.POS2) {
            ...
            BuildDynasty.getBuilderProcess().build("Fill", composite, origin);
            ...
            BuildDynasty.getBuilderProcess().build("Fill", clipboard, pos.add(clipboardOffset));
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The sel command allows you to manipulate BuildDynasty's selections, similarly to WorldEdit.",
                "",
