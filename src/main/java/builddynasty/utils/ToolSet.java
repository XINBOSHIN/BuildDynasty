...
import BuildDynasty.utils.accessor.IItemTool;
...
public class ToolSet {
    ...
    public ToolSet(EntityPlayerSP player) {
        ...
        if (BuildDynasty.settings().considerPotionEffects.value) {
    ...
    public int getBestSlot(Block b, boolean preferSilkTouch, boolean pathingCalculation) {
        ...
        */
        if (!BuildDynasty.settings().autoTool.value && pathingCalculation) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (!BuildDynasty.settings().useSwordToMine.value && itemStack.getItem() instanceof ItemSword) {
                continue;
            }

            if (BuildDynasty.settings().itemSaver.value && (itemStack.getItemDamage() + BuildDynasty.settings().itemSaverThreshold.value) >= itemStack.getMaxDamage() && itemStack.getMaxDamage() > 1) {
                continue;
            }
    ...
    private double avoidanceMultiplier(Block b) {
        return BuildDynasty.settings().blocksToAvoidBreaking.value.contains(b) ? BuildDynasty.settings().avoidBreakingMultiplier.value : 1;
    }
