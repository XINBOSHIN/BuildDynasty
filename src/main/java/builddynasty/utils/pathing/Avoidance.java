...
public class Avoidance {
    ...
    public static List<Avoidance> create(IPlayerContext ctx) {
        if (!BuildDynasty.settings().avoidance.value) {
            return Collections.emptyList();
        }
        List<Avoidance> res = new ArrayList<>();
        double mobSpawnerCoeff = BuildDynasty.settings().mobSpawnerAvoidanceCoefficient.value;
        double mobCoeff = BuildDynasty.settings().mobAvoidanceCoefficient.value;
        if (mobSpawnerCoeff != 1.0D) {
            ctx.worldData().getCachedWorld().getLocationsOf("mob_spawner", 1, ctx.playerFeet().x, ctx.playerFeet().z, 2)
                    .forEach(mobspawner -> res.add(new Avoidance(mobspawner, mobSpawnerCoeff, BuildDynasty.settings().mobSpawnerAvoidanceRadius.value)));
        }
        if (mobCoeff != 1.0D) {
            ctx.world().loadedEntityList.stream()
                    ...
                    .forEach(entity -> res.add(new Avoidance(new BlockPos(entity), mobCoeff, BuildDynasty.settings().mobAvoidanceRadius.value)));
        }
        return res;
    }
